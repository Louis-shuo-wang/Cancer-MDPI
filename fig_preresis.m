...
%     end
% 
%     text(0.48, 0.91, sprintf('$p_r = %.1f$', pr_values(i)), ...
%     'Units', 'normalized', ...
...
% 
%     if i == 1
%         title(sprintf('Tumor Cell Number'), 'FontSize', 15);
%     end
...
tiledlayout(4, 3, 'TileSpacing','tight','Padding','none');
...
for col_idx = 1:length(pr_values)
    ...
    for row_idx = 1:4
        figure(fig2);
        new_subplot = nexttile((row_idx - 1) * 3 + col_idx);
        ...
        if found
            ...
            set(lgd, 'FontSize', 14, 'Location', 'best');
            axis(new_subplot, 'equal');

            % 坐标刻度和字体
            set(new_subplot, 'XTick', 0:0.2:1, 'YTick', 0:0.2:1, ...
                             'FontSize', 12);
            ...
            xlabel(new_subplot, 'x', 'FontSize', 12);
            ylabel(new_subplot, 'y', 'FontSize', 12);
        end
    end
end
