...
%The fig1 and fig2 are used to produce Figure. 9 and 10 in manuscript.
%The fig5 and fig7 are used to produce Figure. 11 and 12 in manuscript.
%The original figures that the file paths direct can be generated by the
%code named plot_mutspon.
...
% 
%     if i == 1
%         title(sprintf('Oxygen Consumption'), 'FontSize', 15);
%     end
% 
%     % Add mutation rate label at top-right in LaTeX
...
% 
%     if i == 1
%         title(sprintf('Proliferation Rate'), 'FontSize', 15);
%     end
% 
%     lighting phong;
%     camlight('headlight');
...
tlo5 = tiledlayout(4,3, 'TileSpacing','tight','Padding','none');
...
for col_idx = 1:length(mut_rates) - 1
    ...
    for row_idx = 1:4
        ...
        if any(valid_tumor)
            ...
            % Get actual min/max values for this time point
            current_min = min(prate);
            current_max = max(prate);

            % Create colormap with 10 bins between observed min/max
            edges = linspace(current_min, current_max, 11);
            ...
            scatter(pos_tumor(:,1), pos_tumor(:,2), 10, cmap(bin_idx, :), 'filled');
            colormap(cmap);
            clim([current_min current_max]);

            % Enhanced colorbar labeling
            if col_idx == 3
                cb = colorbar;
                cb.Label.String = 'Proliferation Rate';
                cb.Label.FontSize = 12;
                % Add min/max values to colorbar
                cb.Ticks = [current_min, current_max];
                cb.TickLabels = {sprintf('Min: %.2f',current_min), sprintf('Max: %.2f',current_max)};
            end
        ...
        % Only show y-axis info for first column
        if col_idx == 1
            ylabel(ax, sprintf('Time: t = %.1f', 14 + row_idx * 10), 'FontSize', 12);
        else
            set(ax, 'YTickLabel', []);
        ...
        % Only show x-axis info for last row
        if row_idx < 4
            set(ax, 'XTickLabel', []);
        else
            xlabel(ax, 'x', 'FontSize', 12);
        ...
        % Always show ticks for all subplots
        set(ax, 'XTick', 0:0.2:1, 'YTick', 0:0.2:1, ...
                'XTickLabel', 0:0.2:1, 'YTickLabel', 0:0.2:1, ...
                'FontSize', 12);
    end
end
...
for col_idx = 1:length(mut_rates) - 1
    ...
    for row_idx = 1:4
        ...
        if any(valid_tumor)
            pos_tumor = cellfun(@(tc) tc.position, tumor_cells(valid_tumor), 'UniformOutput', false);
            pos_tumor = cat(1, pos_tumor{:});
            dthresh = cellfun(@(tc) tc.death_threshold, tumor_cells(valid_tumor));

            % Get actual min/max values for this time point
            current_min = min(dthresh);
            current_max = max(dthresh);

            % Create colormap with 10 bins between observed min/max
            edges = linspace(current_min, current_max, 11);
            bin_idx = discretize(dthresh, edges);
            cmap = parula(10);
            ...
            if col_idx == 3
                cb = colorbar;
                cb.Label.String = 'Death Threshold';
                cb.Label.FontSize = 12;
                % Add min/max values to colorbar
                cb.Ticks = [current_min, current_max];
                cb.TickLabels = {sprintf('Min: %.2f',current_min), sprintf('Max: %.2f',current_max)};
