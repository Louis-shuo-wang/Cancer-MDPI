...
mut_rt = [1e-1, 1e-2, 1e-3, 1e-4];
%%
for ijk = 1:length(mut_rt)
    filename = 'Figures/vascular/(epsilon = 1e-2)/vascular0/info.mat';
    % filename = fullfile(sprintf('Figures/vascular/mutation_spon/mr_%.1g', mut_rt(ijk)), 'info.mat');
    info = load(filename, 'info').info;
    time_init = info.time;

    params = initializeParameters();
    params.mutation_rate = mut_rt(ijk);
    folderName = sprintf('Figures/vascular/(epsilon = 1e-2)/mutation_spon/mr_%.1g', mut_rt(ijk));
